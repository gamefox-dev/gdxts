...
const MODEL_SCALE = 2.55;
const MODEL_SPACING_X = 5.35;
const MODEL_Y_OFFSET = 0;
const GROUND_Y = -0.22;
const NORMAL_SMOOTH_BLEND = 0.7;
...
const PBR_PRESETS: PBRPreset[] = [
  {
    name: 'Adventure Matte',
    style: {
      baseRoughness: 0.62,
      baseMetallic: 0.0,
      specularLevel: 0.58,
      clearcoatStrength: 0.04,
      clearcoatRoughness: 0.35,
      ambientStrength: 0.31,
      hemiStrength: 0.36,
      skyTint: [0.22, 0.29, 0.46],
      groundTint: [0.03, 0.05, 0.09],
      shadowLift: 0.055,
      rimStrength: 0.025,
      rimPower: 4.2,
      exposure: 1.05,
  ...
  },
  {
    ...
    style: {
      ...
      baseMetallic: 0.0,
      specularLevel: 0.72,
      clearcoatStrength: 0.08,
      clearcoatRoughness: 0.26,
      ambientStrength: 0.29,
      hemiStrength: 0.4,
      skyTint: [0.24, 0.31, 0.5],
      groundTint: [0.02, 0.04, 0.08],
      shadowLift: 0.04,
      rimStrength: 0.035,
      rimPower: 3.8,
      exposure: 1.08,
  ...
  },
  {
    ...
    style: {
      ...
      baseMetallic: 0.0,
      specularLevel: 0.66,
      clearcoatStrength: 0.06,
      clearcoatRoughness: 0.3,
      ambientStrength: 0.3,
      hemiStrength: 0.44,
      skyTint: [0.24, 0.33, 0.54],
  ...
  },
  {
    ...
    style: {
      ...
      ambientStrength: 0.27,
      hemiStrength: 0.47,
      skyTint: [0.23, 0.33, 0.57],
      groundTint: [0.02, 0.04, 0.08],
      shadowLift: 0.035,
      rimStrength: 0.05,
      rimPower: 3.3,
      ...
      saturation: 1.09,
      valueBoost: 1.05,
      pointLightFalloff: 0.22
    }
  }
];

export const createTest3DPBRScreen = async (viewport: Viewport): Promise<Screen> => {
  const gl = viewport.getContext();
  const canvas = viewport.getCanvas();

  const cam = new PerspectiveCamera(57, canvas.width, canvas.height);
  cam.position.set(0, 6.6, 21.8);
  cam.lookAt(0, 1.9, 0);
  cam.near = 0.1;
  cam.far = 300;
  cam.update();
  ...
  const pbrProvider = new PBRShaderProvider(gl, null, PBR_PRESETS[currentPresetIndex].style);
  const modelBatch = new ModelBatch(gl, null, null, pbrProvider);

  const environment = new Environment();
  environment.set(new ColorAttribute3D(ColorAttribute3D.AmbientLight, new Color(0.24, 0.24, 0.26, 1)));
  const keyLight = new DirectionalLight().set(1.25, 1.18, 1.1, -0.45, -0.72, -0.52);
  const fillLight = new DirectionalLight().set(0.5, 0.62, 0.82, 0.72, -0.26, 0.32);
  const rimLight = new DirectionalLight().set(0.42, 0.52, 0.66, 0.18, -0.1, 0.98);
  const leftPoint = new PointLight().setFromValue(0.28, 0.42, 0.7, -7.6, 3.8, 7.2, 2.4);
  const rightPoint = new PointLight().setFromValue(0.66, 0.56, 0.44, 8.1, 3.0, 6.7, 2.0);
  environment.addLight(keyLight);
  environment.addLight(fillLight);
  environment.addLight(rimLight);
  environment.addLight(leftPoint);
  environment.addLight(rightPoint);
  ...
  const setPreset = (index: number) => {
    ...
    if (currentPresetIndex < 0) currentPresetIndex = PBR_PRESETS.length - 1;
    if (currentPresetIndex >= PBR_PRESETS.length) currentPresetIndex = 0;
    pbrProvider.setStyle(PBR_PRESETS[currentPresetIndex].style);
  };
  ...
  const bgTop = new Color(0.04, 0.05, 0.14, 1);
  const bgBottom = new Color(0.0, 0.05, 0.08, 1);
  const floorGlowInner = new Color(0.0, 0.52, 0.62, 0.16);
  const floorGlowOuter = new Color(0.0, 0.35, 0.4, 0.045);
  const shadowCore = new Color(0.0, 0.0, 0.0, 0.28);
  const shadowFeather = new Color(0.0, 0.0, 0.0, 0.12);
  ...
  return {
    update(delta: number, game: Game) {
      ...
      const bgCenterX = camera2d.position.x;
      const bgCenterY = bgBottomY - camera2d.viewportHeight * 0.08;
      ...
      shapeRenderer.circle(true, bgCenterX, bgCenterY, 220, floorGlowOuter, 36);
      shapeRenderer.circle(true, bgCenterX, bgCenterY, 146, floorGlowInner, 28);
      ...
      const presetLabel = `Look: ${PBR_PRESETS[currentPresetIndex].name}`;
