...
export class DefaultRenderableSorter {
  ...
  compare(o1: Renderable, o2: Renderable): number {
    const b1 =
      o1.material.has(BlendingAttribute.Type) &&
      (o1.material.get(BlendingAttribute.Type) as BlendingAttribute).blended;
    const b2 =
    ...
    const dst =
      1000 * this.camera.position.dst2(this.tmpV1) -
      1000 * this.camera.position.dst2(this.tmpV2);
