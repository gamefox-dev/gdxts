...
export class ModelBuilder {
  ...
  private getBuilder(attributes: VertexAttributes): MeshBuilder {
    for (const mb of this.builders)
      if (mb.getAttributes().equals(attributes) && mb.getLastIndex() < SHORT_MAX_VALUE / 2) return mb;
  ...
  public createBox(width: number, height: number, depth: number, material: Material, attributes: number) {
    this.begin();
    this.part('box', GL20.GL_TRIANGLES, attributes, material).box(width, height, depth);
    return this.end();
  }
  ...
  private static rebuildReferences(model: Model, node: Node) {
    for (const mpm of node.parts) {
      if (!model.materials.includes(mpm.material)) model.materials.push(mpm.material);
      if (!model.meshParts.includes(mpm.meshPart)) {
        model.meshParts.push(mpm.meshPart);
        if (!model.meshes.includes(mpm.meshPart.mesh)) model.meshes.push(mpm.meshPart.mesh);
        model.manageDisposable(mpm.meshPart.mesh);
      }
    }
    for (const child of node.getChildren()) this.rebuildReferences(model, child);
  }
}
