...
import { ColorAttribute3D, DirectionalLightsAttribute, PointLightsAttribute, SpotLightsAttribute } from "../../../attributes";
...
export class SceneManager implements Disposable {
	...
	constructor(maxBones = 24) {
		this(PBRShaderProvider.createDefaultWithNumBones(maxBones), PBRShaderProvider.createDefaultDepthtWithNumBones(maxBones));
	}
	...
	public addScene(scene: Scene, appendLights = true){
		...
		if(appendLights){
			for(const e of scene.lights){
				this.environment.addLight(e.value);
			}
		}
	}
