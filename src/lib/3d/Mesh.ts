...
export enum VertexDataType {
 ...
 export class Mesh implements Disposable {
    ...
    autoBind = true;
     ...
     protected Mesh (vertices: VertexData, indices: IndexData, isVertexArray: boolean) {
     ...
     constructor (private gl:WebGL2RenderingContext, staticVertices: boolean, staticIndices: boolean, maxVertices: number, maxIndices: number, attributes: VertexAttributes) {
     ...
     private makeVertexBuffer (isStatic: boolean, maxVertices: number, vertexAttributes: VertexAttributes): VertexData {
     ...
     public updateInstanceDataWithArray (targetOffset: number, source: number[], sourceOffset: number = 0, count: number = -1): Mesh {
        if(count < 0) count = source.length;
        this.instances.updateInstanceData(targetOffset, source, sourceOffset, count);
        return this;
     }

     public updateInstanceDataWithBuffer (targetOffset: number, source: Float32Array, sourceOffset: number = 0, count: number = -1): Mesh {
        if(count < 0) count = source.length;
         this.instances.updateInstanceData(targetOffset, source, sourceOffset, count);
         return this;
     }
     ...
     public getVertices (vertices: number[], srcOffset: number = 0, count: number = -1, destOffset: number = 0): number[] {
        ...
        if ((vertices.length - destOffset) < count) throw new Error(
            "not enough room in vertices array, has " + vertices.length + " floats, needs " + count);
        const pos = getVerticesBuffer().position();
     ...
     public setIndices (indices: number[], offset: number = 0, count: number = -1): Mesh {
     ...
     public getIndices (srcOffset: number, count: number, indices: number[], destOffset: number) {
         ...
         if (srcOffset < 0 || srcOffset >= max || srcOffset + count > max) throw new Error(
             "Invalid range specified, offset: " + srcOffset + ", count: " + count + ", max: " + max);
         if ((indices.length - destOffset) < count) throw new Error(
             "not enough room in indices array, has " + indices.length + " shorts, needs " + count);
         const pos = getIndicesBuffer().position();
     ...
     public render (shader: ShaderProgram, primitiveType: number, offset: number = 0, count: number = -1, autoBind: boolean = this.autoBind) {
        if(count < 0) count = this.indices.getNumMaxIndices() > 0 ? this.getNumIndices() : this.getNumVertices();
         ...
         if (this.isVertexArray) {
             ...
             let numInstances = 0;
             if (this.isInstanced) numInstances = this.instances.getNumInstances();
 
             if (this.indices.getNumIndices() > 0) {
                 if (count + offset > this.indices.getNumMaxIndices()) {
                     throw new Error("Mesh attempting to access memory outside of the index buffer (count: " + count
                         + ", offset: " + offset + ", max: " + this.indices.getNumMaxIndices() + ")");
                 }
     ...
     public calculateRadiusSquared (centerX: number, centerY: number, centerZ: number, offset: number, count: number,
         ...
         case 2:
             for (let i = offset; i < end; i++) {
                 const idx = (index[i] & 0xFFFF) * vertexSize + posoff;
                 this.tmpV.set[i](verts[i], verts[idx + 1], 0);
                 if (transform != null)  this.tmpV.multiply(transform);
                 const r = this.tmpV.sub(new Vector3(centerX, centerY, centerZ)).length2();
                 if (r > result) result = r;
             }
         ...
         case 3:
             for (let i = offset; i < end; i++) {
                 const idx = (index[i] & 0xFFFF) * vertexSize + posoff;
                 this.tmpV.set[i](verts[i], verts[idx + 1], verts[idx + 2]);
                 if (transform != null)  this.tmpV.multiply(transform);
                 const r = this.tmpV.sub(new Vector3(centerX, centerY, centerZ)).length2();
                 if (r > result) result = r;
             }
     ...
     public calculateRadius (centerX: number, centerY: number, centerZ: number, offset: number = 0, count: number = this.getNumIndices(), transform: Matrix4 = null): number {
         return Math.sqrt(this.calculateRadiusSquared(centerX, centerY, centerZ, offset, count, transform));
     }
 
     public getIndicesBuffer (): Int8Array {
     ...
     public static transform (matrix: Matrix4, vertices: number[], vertexSize: number, offset: number, dimensions: number,
        start: number, count: number) {
         if (offset < 0 || dimensions < 1 || (offset + dimensions) > vertexSize) throw new Error();
         if (start < 0 || count < 1 || ((start + count) * vertexSize) > vertices.length) throw new Error(
     ...
     public static transformUV (matrix: Matrix3, vertices: number[], vertexSize: number, offset: number, start: number, count: number) {
         if (start < 0 || count < 1 || ((start + count) * vertexSize) > vertices.length) throw new Error(
             "start = " + start + ", count = " + count + ", vertexSize = " + vertexSize + ", length = " + vertices.length);
     ...
     public copy (isStatic: boolean, removeDuplicates: boolean = false, usage: number[] = null): Mesh {
        ...
        let attrs: VertexAttribute[] = null;
         ...
         let result: Mesh;
         if (attrs == null)
             result = new Mesh(this.gl, isStatic, isStatic, numVertices, indices == null ? 0 : indices.length, this.getVertexAttributes());
         else
             result = new Mesh(this.gl, isStatic, isStatic, numVertices, indices == null ? 0 : indices.length, new VertexAttributes(attrs));
